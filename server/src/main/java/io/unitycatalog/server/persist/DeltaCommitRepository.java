 ...
 *
 * <p>* For example, consider the following sequence of commit operations: 1. commit(v1). Then
 * database has [v1] 2. commit(v2). Then database has [v1, v2] 3. commit(v3). Then database has [v1,
 * v2, v3] 4. backfill(v1). Then database has [v2, v3]. Any version <= v1 is removed. 5. commit(v4)
 * & backfill(v3). Then database has [v4]. Any version <= v3 is removed. 6. commit(v5). Then
 * database has [v4, v5] 6. backfill(v5). Then database has [v5(is_backfilled_latest_commit=true)].
 * v4 is removed. But v5 has to be kept as the last record in database. 6. commit(v6). Then database
 * has [v5(is_backfilled_latest_commit=true), v6]
 */
