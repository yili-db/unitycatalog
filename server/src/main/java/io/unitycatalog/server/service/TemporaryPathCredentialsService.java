...
public class TemporaryPathCredentialsService extends AuthorizedService {
  ...
  private void authorizeForOperation(
    ...
    // 2. if the path is a parent of any governed object, fail with error.
    // 3. if the path is found to be part of any governed object, it can only be one of them:
    ...
    // 4. Otherwise, the path must belong to an external location and user must have access to
    // it.
    ...
    Map<SecurableType, Object> resourceKeys = keyMapper.mapResourceKeys(
        Map.of(SecurableType.METASTORE, "metastore",
            SecurableType.EXTERNAL_LOCATION, generateTemporaryPathCredential.getUrl()));
