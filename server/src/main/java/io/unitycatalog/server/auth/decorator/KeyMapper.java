...
public class KeyMapper {
  ...
  public KeyMapper(Repositories repositories) {
    this.pathBasedRpcUtils = repositories.getPathBasedRpcUtils();
  ...
  public Map<SecurableType, Object> mapResourceKeys(Map<SecurableType, Object> resourceKeys) {
    ...
    if (resourceKeys.containsKey(EXTERNAL_LOCATION)) {
      ...
      if (resourceObject instanceof UUID) {
        ...
        String nameOrPath = (String) resourceKeys.get(EXTERNAL_LOCATION);
        if (nameOrPath.contains("/")) {
          ...
          resourceIds.putAll(pathBasedRpcUtils.getMapResourceIdsForPath(nameOrPath));
        } else {
          // Otherwise it's the name
          resourceIds.put(
              EXTERNAL_LOCATION,
              externalLocationRepository.getExternalLocationDAO(nameOrPath).getId());
        }
      }
    }
