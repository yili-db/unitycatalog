...
trait UCProxyViewSupport { self: UCProxy =>
  ...
  protected[spark] def buildV1ViewTable(t: UCTableInfo): Table = {
    ...
    val queryOut =
      if (base.contains(CatalogTable.VIEW_QUERY_OUTPUT_NUM_COLUMNS)) Map.empty[String, String]
      else UCViewProperties.queryColumnNamesToProps(fields.map(_.name).toSeq)
